...
class GraphAttentionEmbedding(torch.nn.Module):
    ...
    def forward(self, x, last_update, edge_index, t, msg):
        try:
            ...
            print(f"Error in GraphAttentionEmbedding forward pass: {e}")
            return None
...
class LinkPredictor(torch.nn.Module):
    ...
    def forward(self, z_src, z_dst):
        try:
            ...
            z_dst = z_dst.to(device)
            h = torch.cat([self.lin_src(z_src), self.lin_dst(z_dst)], dim=-1)
            h = self.lin_seq(h)
            return h
        except Exception as e:
            print(f"Error in LinkPredictor forward pass: {e}")
            return None
...
def cal_pos_edges_loss_multiclass(link_pred_ratio, labels):
    try:
        ...
        print(f"Error in cal_pos_edges_loss_multiclass: {e}")
